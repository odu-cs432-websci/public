...
**Due:** Sunday, December 4, 2021 by 11:59pm  

**This is an EXTRA CREDIT activity. HW8 is optional.**

***NOTES:***
* Note new `generatetweetvector.py` that includes code to write tweet data out to file so that don't have to collect new tweet data each time when debugging (`writecounts()`, `readcounts()`).  Maybe put this in parse instead.
* write some output statements that give indications of which version being used
* If `generatefeedvector.py` isn't discussed in lecture materials, make sure to note that it's covered in the book and to note the typo below.
* Also note that students should use the `readfile()` in the Colab notebook rather than the one in the book.
* And (same as from HW7), if use code straight from book, will need to adjust print statements (Python2 -> Python 3)
* May need to add a check to make sure that no account is added to tweetdata.txt that has all 0s?  Or at least warn students that it will cause an issue when doing the MDS scaling. I guess this could happen if popularlist isn't computed correctly?
  * check added to https://github.com/odu-cs432-websci/solutions/blob/main/fall21/HW8/generatetweetvector-SOL.py
  * just prints out the account that doesn't contain a word in popularlist, but still outputs that to tweetdata.txt, also updated in https://github.com/odu-cs432-websci/solutions/tree/main/fall21/HW8 
* Typo in line 93 of generatetweetvector.py (carried over from generatefeedvector.py): `if count > 1` → `if count > 0`
  * updated in https://github.com/odu-cs432-websci/solutions/tree/main/fall21/HW8
